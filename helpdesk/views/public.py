...
def view_ticket(request):
    print("view_ticket_public")
    ticket_req = request.GET.get('ticket', None)
    email = request.GET.get('email', None)
    key = request.GET.get('key', None)
    print("uuid: ", ticket_req)
    print("email: ", email)
    print("key: ", key)
    
    # if ticket_req == None and email == None:
    #     return search_for_ticket(request, _('Por favor introduzca el número de ticket y su correo electrónico.'))
    ...
    if not (key and email):
        ...
        if key is None and email is None:
            print("aqui 1")
            return search_for_ticket(request, _('No hubo resultados, intente de nuevo.'))
        else:
            print("aqui 2")
    ...
    try:
        ticket = Ticket.objects.get(secret_key=key, submitter_email__iexact=email)
